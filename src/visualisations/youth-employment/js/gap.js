...
export default function (chart, geoCode, duration) {
  ...
  d3.select('#gap-track')
    ...
    .join(
      enter => enter.append('div')
        .attr('id', 'gap')
        .call(gap => gap.append('div').attr('id', 'gap-value').classed('annotation', true))
  ...
  function gapUpdate(node, duration) {
    node.transition()
      ...
      .select('#gap-value')
        .on('start', function() { this.__start = this.__textValue || 0 })
        .textTween(d => function (t) { 
          const value = Math.abs(Math.round(d[0].x * 100) - Math.round(d[1].x * 100))
          this.__textValue = Math.round(this.__start + (value - this.__start) * t)
          return this.__textValue + '%'
         })
  }

  function convertPoint(point, parent) {
    const [gapX, gapY] = relatedPosition(point.parentNode, parent, [0,  point.__data__.y])
    return Object.assign({gapX: point.__data__.x, gapY: gapY / parent.offsetHeight}, d3.select(point).datum())
  }
